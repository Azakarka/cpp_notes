...
// компилятор не будет ругаться, однако если мы юзаем mutable_, скорее всего у нас плохой код, можно придумать что-нибудь другое
```
...
Что есть this?
```cpp
Указатель очев const, но

в неконстантных методах так
//Vector* const this
...
// класс понимает, что он друг и дает доступ ко всему, что в нем есть
}
```
...
// также такой функции доступны все приватные поля
static double Norm(const std::array<double, 3>& values) {
		Vector vector;
		vector.At(0) = 1;
		vector.At(1) = 2;
...
class Vector {
	...
	// мы от каждого типа как бы вызываем конструктор
...
Print2(100); // неявное приведение типов, size_t к Vector
...
// так уже нельзя неявно приводить
```
...
```cpp
	...
	// делигирование может быть только на одном уровне
	// нельзя смешивать список инициализации и делигирующий конструктор
```
